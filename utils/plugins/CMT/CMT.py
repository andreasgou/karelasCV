...
import numpy as np
import utils.plugins.CMT.util as util
...
class CMT(object):
	...
	def initialise(self, im_gray0, tl, br):
		...
		# Initialise detector, descriptor
		# CV2
		# self.detector = cv2.FeatureDetector_create(self.DETECTOR)
		# self.descriptor = cv2.DescriptorExtractor_create(self.DESCRIPTOR)
		# CV3
		self.detector_descriptor_3 = cv2.BRISK_create()
		# self.detector_descriptor_3 = cv2.ORB_create()
		# Not functional yet, exceptions for improper usage thrown
		# self.detector_descriptor_3 = cv2.xfeatures2d.SIFT_create()
		# self.detector_descriptor_3 = cv2.xfeatures2d.SURF_create(400)
		...
		# CV3
		keypoints_cv = self.detector_descriptor_3.detect(im_gray0, None)
		print("[debug]", "CMT.initialise(): keypoints found")

		# Remember keypoints that are in the rectangle as selected keypoints
		ind = util.in_rect(keypoints_cv, tl, br)
		print("[debug]", "CMT.initialise(): keypoints in rect selected")
		selected_keypoints_cv = list(itertools.compress(keypoints_cv, ind))
		print("[debug]", "CMT.initialise(): keypoints in rect compressed")
		# CV2
		# selected_keypoints_cv, self.selected_features = self.descriptor.compute(im_gray0, selected_keypoints_cv)
		# CV3
		selected_keypoints_cv, self.selected_features = self.detector_descriptor_3.compute(im_gray0, selected_keypoints_cv)
		print("[debug]", "CMT.initialise(): keypoints in rect computed")

		selected_keypoints = util.keypoints_cv_to_np(selected_keypoints_cv)
		print("[debug]", "CMT.initialise(): keypoints in rect as numpy array")
		num_selected_keypoints = len(selected_keypoints_cv)
		print("[debug]", "CMT.initialise(): keypoints in list: {}, keypoints in array: {}"
		      .format(num_selected_keypoints, selected_keypoints.size))
		...
		print("[debug]", "CMT.initialise(): keypoints in background compressed")
		# CV2
		# background_keypoints_cv, background_features = self.descriptor.compute(im_gray0, background_keypoints_cv)
		# CV3
		# background_keypoints_cv, background_features = self.detector_descriptor_3.detectAndCompute(im_gray0, background_keypoints_cv)
		background_keypoints_cv, background_features = self.detector_descriptor_3.compute(im_gray0, background_keypoints_cv)
		print("[debug]", "CMT.initialise(): keypoints in background computed")

		_ = util.keypoints_cv_to_np(background_keypoints_cv)
		print("[debug]", "CMT.initialise(): keypoints in background as numpy array")
		...
		print("[debug]", "CMT.initialise(): classes for keypoints in rect initialized")
		background_classes = zeros(len(background_keypoints_cv))
		print("[debug]", "CMT.initialise(): classes for keypoints in background initialized")

		# Stack background features and selected features into database
		if background_features is not None and self.selected_features is not None:
			self.features_database = vstack((background_features, self.selected_features))
			print("[debug]", "CMT.initialise():", "Feature dimensions: background={}, selected={}"
			      .format(background_features.shape, self.selected_features.shape))
		elif background_features is not None:
			self.features_database = background_features
			print("[debug]", "CMT.initialise():", "Feature dimensions: background={}"
			      .format(background_features.shape))
		elif self.selected_features is not None:
			self.features_database = self.selected_features
			print("[debug]", "CMT.initialise():", "Feature dimensions: selected={}"
			      .format(self.selected_features.shape))
		# self.features_database = vstack((background_features, self.selected_features))
		print("[debug]", "CMT.initialise(): background and in-rect features stacked in db")

		# Same for classes
		self.database_classes = hstack((background_classes, self.selected_classes))
		print("[debug]", "CMT.initialise(): background and in-rect classes stacked in db")
		...
		print("[debug]", "CMT.initialise(): Get all distances between selected keypoints in square form")
		...
		print(self.active_keypoints, type(self.active_keypoints))
	...
	def process_frame(self, im_gray):
		...
		# CV3
		keypoints_cv, features = self.detector_descriptor_3.detectAndCompute(im_gray, None)
