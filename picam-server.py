...
def hw_video_start(dialog, sock, command=[]):
	...
	socs.netThrow(sock, "Initializing remote camera..")
	
	if picam.init_camera(conf=picam_conf, stream=dialog.streams[0].c_fstream) == 'idle':
		socs.netThrow(sock, "- Remote camera initialized")
		socs.netThrow(sock, "- Switching to capture mode..")
		# set caller
		picam.socket = sock
	...
	socs.netThrow(sock, "- Capture started")
	socs.netThrow(sock, "Remote camera state: {}".format(picam.state))
	socs.netThrow(sock, cam.get_camera_settings(picam))
...
def hw_camera_debug(cam):
	...
	print(cam.log)
	socs.netThrow(cam.socket, cam.log)
...
def cmd_listsets(sock, path=None):
	for f in glob.glob("camset*.json"):
		print(f)
		socs.netThrow(sock, f)
...
myServer = socs.DialogServer(port, True)
